...
- Measured in a retained local pre-release rehearsal: seven-sample A/A stayed within 2.06%, the known-optimized canary reached Middle (`+26.13% ns/op`, `+31.03% B/op`, `+17.23% allocs/op`), all six adversarial canaries failed before scoring, and the complete harness finished in 674 seconds.
- Pending before release: repeat the successful calibration against the final immutable `baseline-v3` SHA, retain it outside temporary storage, create the commit/tag, repin the workflow in a follow-up commit, and repeat on the pinned GitHub Actions runner.
